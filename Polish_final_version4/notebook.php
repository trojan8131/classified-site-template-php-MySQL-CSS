...
class Notebook
{
    ...
    public function login($userid, $pass)
    {
        if (!($this->u = $this->user->get($userid))) {
            $this->error = "Błędna nazwa logowania";
            return false;
        }
    ...
    public function change_mail($mail1, $mail2)
    {
        ...
        foreach ($users as $k => $v) {
            if ($v["useremail"] == $mail2) {
                print_r($v["useremail"]);
                $this->error = "Adres jest już zajęty";
                return false;
            }
        }
    ...
    public function register($userid, $username, $email, $pass,$pass1)
    {
        if ($u = $this->user->get($userid)) {
            $this->error .= "Nazwa logowania zajęta.";
            return false;
        }
        if ($pass!=$pass1) {
            $this->error .= "Hasła są różne.";
            return false;
        }
        if ($u = $this->user->get($username,"username")) {
            $this->error .= "Nazwa użytkownika zajęta.";
            return false;
        }
        if ($u = $this->user->get($email,"useremail")) {
            $this->error .= "E-mail jest już zajęty.";
            return false;
        }
    ...
    public function process()
    {
        ...
        if ($this->context == 'add_classified') {
            if (isset($_POST['cla_title'])) {
                ...
                if (count($_FILES['img_file']['name']) > 3) {
                    $_SESSION["error"] = "Za dużo plików, max 3";
                    return false;
                }
                if (!$cities = $this->city->get($_POST['city'], "name")) {
                    $_SESSION["error"] = "Złe miasto";
                    return false;
                }
                ;
        ...
        if ($this->context == 'classifieds') {
            ...
            $data["subcategory"] = $this->subcategory->getAll(false, false, "cat_id desc");
            $_SESSION["error12"]=count($data["classifieds"]);
            $data["counter"]=count($data["classifieds"]);
            if (isset($_GET['page'])) {

                if ($_GET['page'] < 1 || $_GET['page'] > $data["counter"]) $page = 1;
        
                else $page = $_GET['page'];
        
            }
            else $page = 1;
            $data["classifieds"]=array_slice($data["classifieds"],($page-1)*5,5);
            $data["counter2"]=$data["counter"]-$page*5;
            $data["page"]=$page;
            $data["pages"]=0;
            for($i=0;$i<$data["counter"];$i+=5)
            {
                $data["pages"]++;
            }
